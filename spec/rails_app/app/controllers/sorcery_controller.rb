...
class SorceryController < ActionController::Base
  ...
  def login_at_test_openid_connect
    login_at(:openid_connect)
  end
  ...
  def test_login_from_openid_connect
    if @user = login_from(:openid_connect)
      redirect_to 'bla', notice: 'Success!'
    else
      redirect_to 'blu', alert: 'Failed!'
    end
  ...
  def test_return_to_with_external_openid_connect
    if @user = login_from(:openid_connect)
      redirect_back_or_to 'bla', notice: 'Success!'
    else
      redirect_to 'blu', alert: 'Failed!'
